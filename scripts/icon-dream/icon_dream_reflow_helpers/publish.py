...
from .common import load_plan, open_existing_target, s3_map
...
def dataset_encoding(
    ...
    *,
    ...
    """Build the final Zarr encoding for a dataset."""
    compressor = gd.get_compressor(level=compression_level)
    return gd.get_encoding(
        ds,
...
def make_fill_variable(template: "xr.DataArray", name: str) -> "xr.DataArray":
    ...
    arr = xr.DataArray(values, dims=template.dims, coords=template.coords, attrs=template.attrs)
    arr.name = name
    return arr


def pad_missing_existing_vars_for_append(candidate: "xr.Dataset", existing: "xr.Dataset") -> "xr.Dataset":
    ...
    for name, data in existing.data_vars.items():
        if name not in candidate.data_vars:
            candidate[name] = make_fill_variable(data.isel(time=slice(0, candidate.sizes.get("time", 0))), name)
...
def append_time_block(
    ...
    *,
    ...
    new_time_values = [value for value in candidate["time"].values if str(value) not in existing_times]
    if not new_time_values:
        return 0
    append_ds = pad_missing_existing_vars_for_append(candidate.sel(time=new_time_values), existing)
    append_ds.to_zarr(
        s3_map(target_path, s3_options),
        mode="a",
        append_dim="time",
...
def merge_level_dataset(
    ...
    *,
    ...
    if replace_existing_times:
        ...
        if overlap_times:
            rewrite_ds = candidate.sel(time=overlap_times).reindex(time=existing["time"].values)
            rewrite_ds.to_zarr(
                s3_map(target_path, s3_options),
                mode="a",
                consolidated=True,
...
def combine_worker_level_outputs(level_paths: list[str]) -> "xr.Dataset":
    ...
    datasets = [xr.open_zarr(path, consolidated=True) for path in level_paths]
    return datasets[0] if len(datasets) == 1 else xr.combine_by_coords(datasets, combine_attrs="drop_conflicts")
...
def finalize_outputs(
    ...
    *,
    ...
    for result in sorted(
        worker_results,
        key=lambda result: (result.get("time_start") or "", result["variable"], result["item_index"]),
    ):
    ...
    for level, level_paths in sorted(grouped.items()):
        candidate = combine_worker_level_outputs(level_paths)
        if "time" in candidate.dims:
            candidate = candidate.sortby("time")
        level_summary = merge_level_dataset(
            candidate,
        ...
        summary["time_slices_written"] += level_summary["time_slices_written"]
        summary["missing_variables_added"] += level_summary["missing_variables_added"]
        summary["static_writes"] += level_summary["static_writes"]
    return summary
